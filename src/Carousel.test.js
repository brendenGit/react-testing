...
it("hides the left arrow if we are at the 0th idx", function () {
  const { container } = render(
    <Carousel
      photos={TEST_IMAGES}
      title="images for testing"
  ...
  // the left arrow should not be visible
  expect(
    container.querySelector(".bi-arrow-left-circle")
  ).not.toBeInTheDocument();
});

it("hides the right arrow if we are at the last idx", function () {
  const { container } = render(
    <Carousel
      photos={TEST_IMAGES}
      title="images for testing"
  ...
  // the right arrow should not be visible
  expect(
